...
def generate_line_population(line, palette, font, count, include_greedy=False):
    ...
    align_population_lengths(population, calculate_longest_individual(line, palette, font), palette_length=len(palette))
...
def lazy_generate_harmony_line(line, palette, font, pop_count=10, mem_rate=0.8, pa_rate=0.3, bw=2, include_greedy=False):
    ...
    while True:
        generation += 1
        new_harm = HarmonyLineSearch.new_harmony_line(palette, population, mem_rate, pa_rate, bw)
...
class HarmonyLineSearch:
    ...
    def next_line(self, incr):
        ...
        for _ in range(incr):
            new_harm = HarmonyLineSearch.new_harmony_line(self.palette, self.population, self.mem_rate, self.pa_rate, self.bw)
...
class GeneticLineSearch:
    ...
    def new_genetic_line_population(palette, population, mutation_rate=0.3, mutation_bw=2):
        ...
        while len(i_ps) > 0:
            ...
            GeneticLineSearch.genetic_mutation(new1, len(palette), mutation_rate, mutation_bw)
            GeneticLineSearch.genetic_mutation(new2, len(palette), mutation_rate, mutation_bw)
    ...
    def next_line(self, incr):
        self.generation += incr
        for _ in range(incr):
            new_population = GeneticLineSearch.new_genetic_line_population(self.palette, self.population, self.mutation_rate, self.mutation_bw)
...
class ParticleSwarmLineSearch:
    def __init__(self, line, palette, font, pop_count, innertion, cog_coeff, soc_coeff, include_greedy=False):
        self.line = line
        self.palette = palette
        self.font = font
        ...
        self.particles, self.fits = generate_line_population(line, palette, font, pop_count, include_greedy)
    ...
    def init_velocities(self):
        ...
        for _ in range(len(self.particles)):
            v = 2 * len(self.palette) * np.random.random(self.pos_len) - len(self.palette)
            self.velocities.append(v)

    def recalculate_velocity(self, velocity, particle_pos, best_particle_pos, best_global_pos):
        ...
        if (np.linalg.norm(velocity) == 0):
            velocity = 2 * len(self.palette) * np.random.random(self.pos_len) - len(self.palette)
        return velocity
    ...
    def next_line(self, incr):
        ...
        for _ in range(incr):
            for i in range(len(self.particles)):
                self.velocities[i] = self.recalculate_velocity(self.velocities[i], self.particles[i],
                                                               self.best_particle_pos[i], self.best_global_pos)
                self.particles[i] += self.velocities[i]
                
                try:
                    self.fits[i] = evaluate_palette_id_arr(map(int, self.particles[i]), self.palette, self.line, self.font)
                except IndexError:
                    self.fits[i] = 0
                if self.fits[i] > self.best_particle_pos_fit[i]:
                    self.best_particle_pos[i] = copy.deepcopy(self.particles[i])
...
def pool_evolutionary_search(finder_class, line_finders, generations, palette):
    while True:
        with Pool(USE_CPU) as p:
            next_states = p.starmap(finder_class.next_line, zip(line_finders, repeat(generations)))
