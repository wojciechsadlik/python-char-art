...
def img_arr2braille_arr(img_arr: np.ndarray, img_colors=256) -> list[list[str]]:
    color_threshold = img_colors // 2
    braille_arr = []
    for y in range(3, img_arr.shape[0], 4):
        braille_arr.append([])
        for x in range(1, img_arr.shape[1], 2):
            braille_unicode_offset = int(img_arr[y-3][x-1] < color_threshold)
            braille_unicode_offset += int(img_arr[y-2][x-1] < color_threshold) << 1
            braille_unicode_offset += int(img_arr[y-1][x-1] < color_threshold) << 2
            braille_unicode_offset += int(img_arr[y-3][x] < color_threshold) << 3
            braille_unicode_offset += int(img_arr[y-2][x] < color_threshold) << 4
            braille_unicode_offset += int(img_arr[y-1][x] < color_threshold) << 5

            braille_unicode_offset += int(img_arr[y][x-1] < color_threshold) << 6
            braille_unicode_offset += int(img_arr[y][x] < color_threshold) << 7

            braille_unicode_offset ^= 0b011111111
            braille_arr[-1].append(chr(BRAILLE_UNICODE_START + braille_unicode_offset))
