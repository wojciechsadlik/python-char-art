...
class HarmonyLineSearch:
    def __init__(self, line, palette, font, pop_count, mem_rate, pa_rate, bw, include_greedy=False):
        ...
        self.generation = 0
        self.el_len = len(self.population[0])

    def next_line(self, incr):
        self.generation += incr
        for _ in range(incr):
            new_harm = []
            while len(new_harm) < self.el_len:
                if (random.random() < self.mem_rate):
                    new_pitch = self.population[random.randrange(0, self.pop_count)][len(new_harm)]
                    if (random.random() < self.pa_rate):
                        new_pitch += random.randint(-self.bw, self.bw) // 2
                        new_pitch = new_pitch % len(self.palette)
                    new_harm.append(new_pitch)
                else:
                    new_harm.append(random.randrange(0, len(self.palette)))
            
            insert_into_sorted_population(self.population, self.fits, new_harm, self.palette, self.line, self.font)
        
        return self.population[0], self.fits[0]
...
def pool_harmony_search(img, palette, font, generations, pop_count=10, mem_rate=0.8, pa_rate=0.3, bw=2, include_greedy=False):
    ...
    while True:
        text_arr = []
        with Pool(USE_CPU) as p:
            l_p_ids = p.starmap(HarmonyLineSearch.next_line, zip(line_finders, repeat(generations)))
        l_p_ids = list(l[0] for l in l_p_ids)
        l_text_arr = starmap(palette_id_arr_to_text_arr, zip(l_p_ids, repeat(palette)))
        text_arr = list(l + '\n' for l in map(''.join, l_text_arr))
        yield text_arr
